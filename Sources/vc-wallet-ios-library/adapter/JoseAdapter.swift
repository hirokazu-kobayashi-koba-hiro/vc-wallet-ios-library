...
public final class JoseAdapter: Sendable {
  ...
  public func sign(
    ...
    let (kid, secKey) = try SecKey.convertFromPrivateKeyFormattedJwk(
  ...
  public func verify(jws: String, publicKeyAsJwk: String) throws -> VerifiedJose {
    ...
    guard
      let header = try? JSONSerialization.jsonObject(with: verifiedJws.header.data(), options: [])
        as? [String: Any],
      let payload = try? JSONSerialization.jsonObject(with: verifiedJws.payload.data(), options: [])
        as? [String: Any]
    else {
...
extension SecKey {
  ...
  static func convertFromPrivateKeyFormattedJwk(algorithm: SignatureAlgorithm, privateKey: String)
    throws -> (String?, SecKey)
  {
  ...
  static func convertFromPublicKeyFormattedJwk(algorithm: SignatureAlgorithm, publicKey: String)
    throws -> SecKey
  {
...
func convertECPrivateKey(privateKeyAsJwk: String) throws -> (String?, SecKey) {
  ...
  guard let crv = jwk["crv"],
    ...
    let dData = Data(base64URLEncoded: d)
  else {
